...
- **ソース追跡**: `/setup` でインストールしたスキルのソース追跡は、`gh skill install` が SKILL.md frontmatter に自動注入するメタデータ（ソースリポジトリ・tree SHA）で行う。skills-lock.json には記録しない
- **gh skill コマンド**: GitHub CLI 組み込みの preview 機能。`gh skill search` で公開リポジトリの SKILL.md を検索し、`gh skill install` でインストールする。/setup はこれを単一の検索・インストールソースとして使用する設計
- **既存スペックとの関連**:
...
3. スキル検索（`gh skill search` 単一ソース）
4. 対話的選択（star 数降順ランキング表示 + インストール先選択）
5. インストール（`gh skill install`）
6. 追加キーワード検索（ループ）
...
スキル検索は GitHub CLI の `gh skill search` を単一ソースとして使用する（`gh skill` は preview 機能であり、予告なく変更される可能性がある）:

| ソース | 検索方法 | ランキング指標 | 表示 |
|---|---|---|---|
| gh skill search | `gh skill search "{query}" --limit 15 --json skillName,repo,description,stars` | star 数降順 | 上位15件 |

`gh skill search` は GitHub Code Search API で公開リポジトリの SKILL.md を検索し、関連度順で結果を返す。表示時は star 数降順でソートし、各スキルに skillName・repo・description・stars を明記する。

フォールバック戦略:
- gh 未インストール時はインストール手順（https://cli.github.com/）を案内し、手動入力に遷移
- gh 未認証時は `gh auth login` を案内（Code Search API は認証必須）
- 古い gh で `gh skill` サブコマンドが存在しない場合は GitHub CLI のアップグレードを案内
- 検索 API エラー時はリトライまたは手動入力をユーザーに確認

### 4. SKILL.md の取得と配置

`gh skill install <repository> [<skill>]` がリポジトリ内の SKILL.md の探索と配置を行うため、ソースごとの取得パス定義は不要。インストール前の内容確認には `gh skill preview <repository> [<skill>]` を使用する。

### 5. セキュリティ検証

Dependency Inversion 原則に基づき、外部スキルのインストールは「信頼の検証」を挟む:
- インストール前に SKILL.md の内容を要約表示
- ソース URL、star 数、最終更新日を明示表示
- ユーザーが明示的に承認してからインストール実行
- Premature Abstraction を避け、現時点ではシンプルな承認フローのみ実装

### 6. skill-creator の同梱

Forge リポの `skills/skill-creator/` ディレクトリに skill-creator の SKILL.md を配置する。install.sh の既存ロジック（`FORGE_DIRS` に `skills` が含まれている）により、自動的に `~/.claude/skills/skill-creator/` にシンボリックリンクが作成される。
...
`/setup` でインストールしたスキルのソース追跡は、`gh skill install` が SKILL.md frontmatter に自動注入するメタデータ（ソースリポジトリ・tree SHA）で行う。skills-lock.json 等の別ファイルでの記録は行わない。
...
### gh skill の preview ステータス
`gh skill` は GitHub CLI の preview 機能であり、予告なく変更される可能性がある。コマンド体系の変更時はコマンド定義の追従更新が必要。

### gh 未インストール・旧バージョン時の graceful degradation
スキル検索・インストールは `gh skill` に依存する。gh が未インストール、または旧バージョンで `gh skill` サブコマンドが存在しない環境では、インストール手順またはアップグレードを案内し、手動入力に遷移する。エラーとして処理を中断しない。

### GitHub API レート制限と認証
`gh skill search` が使用する Code Search API は認証必須。未認証時は `gh auth login` を案内する。レート制限エラー時はリトライまたは手動入力にフォールバックする。
...
### リポジトリごとのディレクトリ構造差異
リポジトリごとにスキルの格納パスは異なるが、`gh skill install` が SKILL.md の探索と配置を行うため、コマンド定義側でソースごとの取得パスを定義する必要はない。
...
### スキル作成提案の閾値
「検索結果が0件、または全ての結果が star 数 100 未満」をスキル作成提案のトリガーとする。この閾値は GitHub のエコシステム成長に応じて将来的に調整が必要となる可能性がある。
